...
class ProductionMonitor:
    ...
    def log_request(self, endpoint_type: str, processing_time: float, success: bool, 
                   input_length: int, audio_length_seconds: Optional[float] = None,
                   memory_usage_gb: Optional[float] = None, request_id: Optional[str] = None):
        """Log a request event."""
        with self.lock:
            event = RequestEvent(
                timestamp=time.time(),
    ...
    def log_error(self, error_type: str, error_message: str, endpoint_type: str,
                  stack_trace: Optional[str] = None, user_input_length: Optional[int] = None,
                  request_id: Optional[str] = None):
        """Log an error event."""
        with self.lock:
            event = ErrorEvent(
    ...
    def send_alert(self, alert_message: str, alert_type: str = "general"):
        ...
        if self.webhook_url:
            try:
                import requests
                payload = {
                    "text": f"🚨 Higgs Audio Alert [{alert_type}]: {alert_message}",
                    "timestamp": time.time()
                }
...
def monitor_request(endpoint_type: str, processing_time: float, success: bool, 
                   input_length: int, **kwargs):
