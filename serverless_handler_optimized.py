...
class OptimizedAudioGenerator:
    ...
    def _build_messages(self, scene_prompt: Optional[str], ref_audio: Optional[str]) -> List[Dict]:
        """Build message list"""
        messages = []
        ...
        if scene_prompt:
            messages.append({
                "role": "system", 
                "content": f"Scene: {scene_prompt}. Generate appropriate speech."
            })
        
        if ref_audio and ref_audio != "default":
            messages.append({
                "role": "user",
                "content": f"Voice reference: {ref_audio}"
            })
