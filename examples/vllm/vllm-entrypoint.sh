...
echo "🚀 Starting vLLM Higgs Audio V2 Container..."
...
if command -v nvidia-smi &> /dev/null; then
    ...
    if [ "$GPU_MEMORY" -lt 24000 ]; then
        echo "⚠️ WARNING: GPU has less than 24GB memory. Performance may be degraded."
    fi
else
...
echo "🏗️ Running vLLM runtime bootstrap..."
./vllm-bootstrap.sh || {
    echo "❌ vLLM Bootstrap failed"
    exit 1
}

echo "✅ vLLM Bootstrap completed successfully!"
...
# Download models at runtime (dependencies now installed)
echo "📥 Downloading models at runtime..."
python /app/download_models.py --model ${MODEL_PATH} || {
    echo "⚠️ Warning: Model download failed, vLLM will attempt to download during startup"
}
