...
- **Tech Stack**: Python, PyTorch, Transformers, HuggingFace, Audio Processing (librosa/torchaudio)
- **Main File**: boson_multimodal/model/higgs_audio/modeling_higgs_audio.py (2289 lines)
- **Core Mechanic**: Multimodal AI model for expressive audio generation with zero-shot voice cloning
- **Key Integration**: HuggingFace Hub, vLLM inference engine, CUDA optimization, AWS S3
- **Platform Support**: Linux, NVIDIA Docker containers, 24GB+ GPU requirement
- **DO NOT**: Modify audio tokenizer architecture, skip memory optimizations, ignore delay patterns
...
## Quick Reference
**HiggsAudio Model**: `boson_multimodal/model/higgs_audio/modeling_higgs_audio.py:1-2289` - Core multimodal architecture  
**Serve Engine**: `boson_multimodal/serve/serve_engine.py:1-50+` - Main inference and API engine  
**Audio Tokenizer**: `boson_multimodal/audio_processing/higgs_audio_tokenizer.py:43-50+` - Audio encoding/decoding  
**Generation CLI**: `examples/generation.py:1-50+` - Command-line generation interface  
**Data Types**: `boson_multimodal/data_types.py` - ChatML and audio content structures  
**Model Config**: `boson_multimodal/model/higgs_audio/configuration_higgs_audio.py` - HiggsAudio configuration  
**Audio Head**: `boson_multimodal/model/higgs_audio/audio_head.py` - Audio decoder projector  
**Data Collator**: `boson_multimodal/data_collator/higgs_audio_collator.py` - Training data batching  
**ChatML Dataset**: `boson_multimodal/dataset/chatml_dataset.py` - Dataset processing pipeline  
**vLLM Server**: `examples/vllm/run_chat_completion.py` - High-throughput inference server  
**Audio Utils**: `boson_multimodal/model/higgs_audio/utils.py` - Audio processing utilities  
**Quantization**: `boson_multimodal/audio_processing/quantization/` - Vector quantization modules  
**Constants**: `boson_multimodal/constants.py` - Model constants and placeholders

## Current State
- [x] Higgs Audio v2 model architecture - Complete
- [x] Zero-shot voice cloning - Complete  
- [x] Multi-speaker dialog generation - Complete
- [x] vLLM high-throughput inference - Complete
- [x] HuggingFace integration - Complete
- [x] Docker deployment support - Complete
- [x] Audio tokenizer with semantic/acoustic features - Complete
- [x] DualFFN architecture - Complete
- [ ] Performance optimization and memory efficiency - Ongoing
- [ ] Extended language support - Planned
- [ ] Real-time streaming generation - Planned

## Development Workflow
1. **Environment Setup**: Launch NVIDIA Docker container with `nvcr.io/nvidia/pytorch:25.02-py3`
2. **Install Dependencies**: Run `pip install -r requirements.txt && pip install -e .`
3. **Test Basic Generation**: Execute `python examples/generation.py --transcript "test audio"`
4. **Model Development**: Modify core components in `boson_multimodal/model/higgs_audio/`
5. **Audio Processing**: Update tokenizer/quantization in `boson_multimodal/audio_processing/`
6. **Integration Testing**: Test changes through `HiggsAudioServeEngine` and vLLM server

## Task Templates
### 1. Voice Sample Integration
1. Add audio file to `examples/voice_prompts/[name].wav`
2. Create text description in `examples/voice_prompts/[name].txt`
3. Test with `python examples/generation.py --ref_audio [name] --transcript "test"`
4. Update voice profile in `examples/voice_prompts/profile.yaml`
5. Verify compatibility with multi-speaker generation
...
3. Test architecture changes with `examples/generation.py`
4. Run performance benchmarks and memory profiling
5. Update technical documentation in `tech_blogs/ARCHITECTURE_BLOG.md`

### 3. Audio Processing Pipeline Updates
1. Modify tokenizer in `boson_multimodal/audio_processing/higgs_audio_tokenizer.py:43+`
2. Update quantization modules in `boson_multimodal/audio_processing/quantization/`
3. Test encoding/decoding with sample audio files
4. Verify backward compatibility with existing model checkpoints
5. Update tokenizer documentation in `tech_blogs/TOKENIZER_BLOG.md`

### 4. vLLM Integration Enhancement
1. Update vLLM server in `examples/vllm/run_chat_completion.py`
2. Test OpenAI-compatible API endpoints
3. Benchmark performance improvements vs standard inference
4. Update vLLM documentation in `examples/vllm/README.md`
5. Verify deployment compatibility with containerized environments

## Anti-Patterns (Avoid These)
❌ **Don't modify audio tokenizer architecture without understanding codec design** - Breaks model compatibility  
❌ **Don't skip CUDA memory optimization for 24GB+ requirements** - Causes OOM errors on production GPUs  
❌ **Don't ignore delay pattern in multi-speaker generation** - Results in incorrect speaker timing  
❌ **Don't modify vector quantization without extensive testing** - Severely degrades audio quality  
❌ **Don't hardcode sampling rates (16kHz vs 24kHz)** - Different models have different requirements  
❌ **Don't bypass HiggsAudioServeEngine for inference** - Skips critical preprocessing and optimization

## Version History
- **v2.0.0** - Higgs Audio v2 with DualFFN architecture and unified tokenizer
- **v1.0.0** - Initial Higgs Audio v1 understanding model
- **Current** - Open source release with vLLM integration and multi-speaker capabilities
[Link major versions to journal entries when JOURNAL.md is established]
